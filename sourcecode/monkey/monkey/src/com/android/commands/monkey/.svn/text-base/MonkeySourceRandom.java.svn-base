...
public class MonkeySourceRandom implements MonkeyEventSource{    
    ...
    private long mThrottleTouch = 0;
    private long mThrottleKey = 0;
    private int EVENT_OF_KEY = 0;
    private int EVENT_OF_TOUCH = 1;
    ...
    public MonkeySourceRandom(long seed, ArrayList<ComponentName> MainApps, long throttle, long throttleTouch, long throttleKey) {
        ...
        mMainApps = MainApps;
        mThrottle = throttle;
        mThrottleTouch = throttleTouch;
        mThrottleKey = throttleKey;
    }
    ...
    private void addThrottle(int eventType) {
    	if (eventType == EVENT_OF_KEY) {
    		if (mThrottleKey > 0)
    			mQ.addLast(new MonkeyThrottleEvent(MonkeyEvent.EVENT_TYPE_THROTTLE, mThrottleKey));
    		else
    			mQ.addLast(new MonkeyThrottleEvent(MonkeyEvent.EVENT_TYPE_THROTTLE, mThrottle));
    	} else if (eventType == EVENT_OF_TOUCH) {
    		if (mThrottleTouch > 0)
    			mQ.addLast(new MonkeyThrottleEvent(MonkeyEvent.EVENT_TYPE_THROTTLE, mThrottleTouch));
    		else
    			mQ.addLast(new MonkeyThrottleEvent(MonkeyEvent.EVENT_TYPE_THROTTLE, mThrottle));
    	}
        //mQ.addLast(new MonkeyThrottleEvent(MonkeyEvent.EVENT_TYPE_THROTTLE, mThrottle));
